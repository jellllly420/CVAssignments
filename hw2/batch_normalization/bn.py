...
def bn_backward(dout, cache):
    ...
    dbeta = (dout * 1).sum(axis = 0)
    dx_hat = dout * cache[0]
    dsample_var = (dx_hat * (cache[1] - cache[2]) * (-0.5) * np.power(cache[3] + eps, -1.5)).sum(axis = 0)
    dsample_mean = (dx_hat * (-1) / np.sqrt(cache[3] + eps)).sum(axis = 0) + dsample_var * ((-2) * (cache[1] - cache[2])).mean(axis = 0)
    dx = dx_hat / np.sqrt(cache[3] + eps) + dsample_var * 2 * (cache[1] - cache[2]) / cache[1].shape[0] + dsample_mean / cache[1].shape[0]
    #----------------TODO------------------
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        mean = alpha * mean + (1 - alpha) * cache[2]
        var = alpha * var + (1 - alpha) * cache[3]
